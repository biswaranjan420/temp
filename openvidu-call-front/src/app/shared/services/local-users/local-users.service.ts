...
export class LocalUsersService {
  ...
  private _webcamVideoActive = <BehaviorSubject<boolean>>new BehaviorSubject(true);
	...
	initialize() {
		...
		this.webcamUser?.setParticipantsMenu(false);
	...
	enableScreenUser(screenPublisher: Publisher) {
		...
		this.screenUser.setParticipantsMenu(this.webcamUser.getParticipantsMenu());
	...
	resetToolbarBtn(){
		...
		this._webcamVideoActive.next(true);
	}
	...
	setParticipantsMenu(menu: boolean) {
		   this.screenUser?.setParticipantsMenu(menu);
		  this.webcamUser?.setParticipantsMenu(menu);
	}
	getParticipantsMenu():boolean{
		
		return  this.webcamUser?.getParticipantsMenu();
	}
}
