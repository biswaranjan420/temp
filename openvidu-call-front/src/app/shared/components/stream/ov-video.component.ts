...
@Component({
	selector: 'ov-video',
	template: `
		<img *ngIf="!_streamManager?.stream?.videoActive" class="poster_img" alt="MeetMonk Logo" src="assets/images/poster.png" />
		<video
			#videoElement
	...
	set streamManager(streamManager: StreamManager) {
		setTimeout(() => {
			...
			if (!!this._videoElement && this._streamManager) {
				if (this._streamManager.stream.typeOfVideo === VideoType.CUSTOM) {
